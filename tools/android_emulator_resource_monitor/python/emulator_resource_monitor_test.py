...
from emulator_resource_monitor import get_val, get_devices, RE_ANSI, RE_FLOAT, RE_PCT
...
"""
    ...
    def test_get_guest_stats_success(self, mock_run_cmd):
        ...
        model, is_booting, total_cpu, adj_cpu, ram, swap = get_guest_stats("emulator-5554", None, True, "device")
        self.assertIn("Mock Phone", model)
        self.assertIn("Android", model)
        self.assertFalse(is_booting)
        self.assertEqual(total_cpu, "200%")
        self.assertIn("50.0%", ram) # Used = Total - Available = 2GB / 4GB = 50%
    ...
    def test_get_guest_stats_resolution_retry(self, mock_run_cmd):
        ...
        model, is_booting, total_cpu, adj_cpu, ram, swap = get_guest_stats("emulator-5554", "Android 12 | Res: N/A", True, "device")
        ...
        model, is_booting, total_cpu, adj_cpu, ram, swap = get_guest_stats("emulator-5554", "Android 12 | Res: N/A", False, "device")
    ...
    def test_get_guest_stats_boot_detection_self_correction(self, mock_run_cmd):
        ...
        model, is_booting, total_cpu, adj_cpu, ram, swap = get_guest_stats("emulator-5554", "Android 12", False, "device")
    ...
    def test_get_guest_stats_unsafe_separator(self, mock_run_cmd):
        ...
        self.assertIn("50.0%", ram) # RAM should still be parsed because it's in parts[0]
        self.assertIn("N/A data", ram) # Disk should be N/A because it couldn't find '/data' in parts[1]

    def test_get_guest_stats_offline_returns_booting(self):
        from emulator_resource_monitor import get_guest_stats
        model, is_booting, total_cpu, adj_cpu, ram, swap = get_guest_stats("emulator-5554", "Android 12", False, "offline")
        self.assertTrue(is_booting)
    ...
    def test_get_guest_stats_busy_device_retains_booting(self, mock_run_cmd):
        ...
        from emulator_resource_monitor import get_guest_stats
        model, is_booting, total_cpu, adj_cpu, ram, swap = get_guest_stats("emulator-5554", "Android 12", True, "device")
    ...
    def test_get_process_stats_mocked(self, mock_iter):
        ...
        mock_proc.num_ctx_switches.return_value = MagicMock(voluntary=100, involuntary=50)
    ...
    def test_get_process_stats_avd_extraction(self, mock_iter):
        ...
        mock_proc.cmdline.return_value = ['emulator', '-avd', 'Pixel_6_API_31', '-port', '5554']
        mock_proc.num_ctx_switches.return_value = MagicMock(voluntary=100, involuntary=50)
        mock_proc.memory_info.return_value = MagicMock(rss=3000000000)
        mock_proc.num_threads.return_value = 254
        mock_proc.cpu_percent.return_value = 15.0
    ...
    def test_get_host_stats_expansion(self, mock_cpu, mock_swap, mock_mem, mock_temps, mock_disk):
        mock_cpu.return_value = 50.0
        mock_mem.return_value = MagicMock(percent=60.0, used=6000000000, total=10000000000)
        mock_swap.return_value = MagicMock(percent=10.0, used=1000000000, total=10000000000)
        ...
        self.assertIn("Read: 0.0 MB/s", res[14])  # initial iteration elapsed gap triggers 0.0 MB/s naturally safely
    ...
    def test_save_summary_coverage(self):
        ...
        mock_summary = {
            ...
            'host_cpu_iterations': 1, 'host_cpu_sum': 45.0, 'host_cpu_max': 45.0,
            'host_ram_iterations': 1, 'host_ram_sum': 60.0, 'host_ram_max': 60.0,
            ...
            'emulators': {},
            'time_series': [{'time': 1000.0, 'host_cpu': 45.0, 'host_ram': 60.0}]
        }
        # Should run without throwing Exceptions smoothly
        with tempfile.TemporaryDirectory() as tmpdir:
            try:
                save_summary(mock_summary, 1005.0, save_csv=True, out_dir=tmpdir)
    ...
    def test_is_virtual_machine_coverage(self):
        from emulator_resource_monitor import save_summary
        mock_summary = {
            'iterations': 1, 'start_time': 1000.0, 'host_cpu_iterations': 1, 'host_cpu_sum': 45.0, 'host_cpu_max': 45.0,
            'host_ram_iterations': 1, 'host_ram_sum': 60.0, 'host_ram_max': 60.0,
            'netsim_cpu_iterations': 0, 'netsim_cpu_sum': 0.0, 'netsim_cpu_max': 0.0,
            'netsim_ram_iterations': 0, 'netsim_ram_sum': 0.0, 'netsim_ram_max': 0.0,
            'emulators': {}, 'time_series': []
        }
    ...
    def test_main_once_flag(self):
        ...
        with patch('sys.argv', ['emulator_resource_monitor.py', '--once', '--no-summary']):
            with patch('emulator_resource_monitor.get_devices', return_value={}), \
                ...
                except Exception as e: self.fail(f"Main threw Exception: {e}")

    def test_main_headless_flag(self):
        from emulator_resource_monitor import main
        with patch('sys.argv', ['emulator_resource_monitor.py', '--headless', '--once', '--no-summary']):
            with patch('emulator_resource_monitor.get_devices', return_value={}), \
                 patch('emulator_resource_monitor.get_host_stats', return_value=("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {})), \
                 patch('os._exit', return_value=None):
                try:
    ...
    def test_main_csv_flag(self):
        from emulator_resource_monitor import main
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch('sys.argv', ['emulator_resource_monitor.py', '--csv', '-o', tmpdir, '--once', '--no-summary']):
                with patch('emulator_resource_monitor.get_devices', return_value={}), \
                     patch('emulator_resource_monitor.get_host_stats', return_value=("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {})), \
                     patch('os._exit', return_value=None):
                    try:
    ...
    def test_main_out_dir_flag(self):
        from emulator_resource_monitor import main
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch('sys.argv', ['emulator_resource_monitor.py', '-o', tmpdir, '--once']):
                with patch('emulator_resource_monitor.get_devices', return_value={}), \
                     patch('emulator_resource_monitor.get_host_stats', return_value=("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {})), \
                     patch('os._exit', return_value=None):
                    try:
    ...
    def test_main_avd_name_ui_connected(self, mock_host, mock_devices, mock_guest, mock_print):
        from emulator_resource_monitor import main
        ...
        mock_host.return_value = ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {"emulator-5554": "Pixel_7_API_36"})

        # Mock guest stats
        mock_guest.return_value = ("Pixel_7_API_36", False, "10%", "5%", "50.0%", "0.0 MB")
        ...
        disp, tags, g_diff, g_real, g_ram, g_swap = guest_results["emulator-5554"]
    ...
    @patch('emulator_resource_monitor.get_devices')
    @patch('emulator_resource_monitor.get_host_stats')
    def test_main_qemu_stats_display(self, mock_host, mock_devices, mock_guest, mock_print):
        from emulator_resource_monitor import main

        mock_devices.return_value = {"emulator-5554": "device"}
        mock_host.return_value = (
            "10%", {"emulator-5554": ("50.0%", "100MB")}, "2%", "30%", "0%", 0, 0, [], "10MB",
            {"emulator-5554": 1234}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A",
    ...
    @patch('emulator_resource_monitor.get_devices')
    @patch('emulator_resource_monitor.get_host_stats')
    def test_main_loop_lookup_by_avd_name(self, mock_host, mock_devices, mock_guest):
        from emulator_resource_monitor import main
        import time
        ...
        mock_guest.return_value = ("Pixel_7_API_36", True, "10%", "5%", "50.0%", "0.0 MB")
        ...
        with patch('time.sleep', side_effect=mock_sleep):
            with patch('sys.argv', ['emulator_resource_monitor.py', '--interval', '0.1', '--no-summary']):
                with patch('os._exit', return_value=None):
                    try:
                        main()
    ...
    @patch('emulator_resource_monitor.get_devices')
    @patch('emulator_resource_monitor.get_host_stats')
    def test_main_loop_migration_serial_to_avd(self, mock_host, mock_devices, mock_guest, mock_save):
        from emulator_resource_monitor import main
        import time
        ...
        mock_host.side_effect = [
            ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {}),
            ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {"emulator-5554": "Pixel_7_API_36"}),
            ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {"emulator-5554": "Pixel_7_API_36"})
        ]
    ...
    @patch('emulator_resource_monitor.get_devices')
    @patch('emulator_resource_monitor.get_host_stats')
    def test_boot_progression_logic(self, mock_host, mock_devices, mock_guest, mock_save):
        from emulator_resource_monitor import main

        mock_devices.return_value = {"emulator-5554": "device"}
        mock_host.return_value = ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {})
        mock_guest.return_value = ("Mock Model", True, "50%", "25%", "50% (2.0GB/4.0GB, 0 errs, 10% data)", "0.0 MB")
    ...
    @patch('emulator_resource_monitor.get_devices')
    @patch('emulator_resource_monitor.get_host_stats')
    def test_boot_progression_logic_non_booting(self, mock_host, mock_devices, mock_guest, mock_save):
        from emulator_resource_monitor import main

        mock_devices.return_value = {"emulator-5554": "device"}
        mock_host.return_value = ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {})
        mock_guest.return_value = ("Mock Model", False, "50%", "25%", "50% (2.0GB/4.0GB, 0 errs, 10% data)", "0.0 MB")
    ...
    @patch('emulator_resource_monitor.get_devices')
    @patch('emulator_resource_monitor.get_host_stats')
    def test_boot_progression_logic_transition(self, mock_host, mock_devices, mock_guest, mock_save):
        from emulator_resource_monitor import main
        ...
        guest_stats_responses = [
            ("Mock Model", True, "50%", "25%", "50% (2.0GB/4.0GB, 0 errs, 10% data)", "0.0 MB"),
            ("Mock Model", False, "50%", "25%", "50% (2.0GB/4.0GB, 0 errs, 10% data)", "0.0 MB")
        ]
        def mock_guest_side_effect(*args, **kwargs):
            if guest_stats_responses:
                return guest_stats_responses.pop(0)
            return ("Mock Model", False, "50%", "25%", "50% (2.0GB/4.0GB, 0 errs, 10% data)", "0.0 MB")
        ...
        self.assertTrue(e_data['track_boot_stats'])
        self.assertFalse(e_data['is_booting']) # But is_booting should be False now
        self.assertGreater(len(e_data['boot_log']), 0)
    ...
    def test_graphs_generation(self, mock_figure_class):
        ...
        mock_summary = {
            ...
            'start_time': 1000.0,
            'start_time_monotonic': 1000.0,
            'host_cpu_iterations': 2, 'host_cpu_sum': 90.0, 'host_cpu_max': 50.0,
            'host_ram_iterations': 2, 'host_ram_sum': 120.0, 'host_ram_max': 60.0,
            'netsim_cpu_iterations': 2, 'netsim_cpu_sum': 4.0, 'netsim_cpu_max': 2.0,
            'netsim_ram_iterations': 2, 'netsim_ram_sum': 20.0, 'netsim_ram_max': 10.0,
            'emulators': {'emulator-5554': {'name': 'Mock Device', 'cpu_iterations': 2, 'cpu_sum': 100.0, 'ram_iterations': 2, 'ram_sum': 100.0, 'ram_max': 50.0}},
            'time_series': [
                {
                    'time': 1000.0, 'host_cpu': 40.0, 'host_ram': 60.0, 'netsim_cpu': 2.0, 'netsim_ram': 10.0,
                    'host_thermal_c': 45.0, 'host_swap': 10.0, 'host_gpu': 20.0, 'host_gpu_mem': 100.0,
                    'host_disk_read_mbs': 5.0, 'host_disk_write_mbs': 2.0,
                    'emu_cpu_tot_emulator-5554': 50.0, 'emu_cpu_adj_emulator-5554': 25.0, 'emu_ram_emulator-5554': 50.0,
                    'qemu_cpu_emulator-5554': 15.0, 'qemu_ram_emulator-5554': 3000.0,
                    'emu_disk_pct_emulator-5554': 10.0, 'emu_errs_emulator-5554': 5.0, 'qemu_cs_emulator-5554': 100.0
                },
                {
                    'time': 1005.0, 'host_cpu': 50.0, 'host_ram': 61.0, 'netsim_cpu': 3.0, 'netsim_ram': 11.0,
                    'host_thermal_c': 46.0, 'host_swap': 11.0, 'host_gpu': 25.0, 'host_gpu_mem': 101.0,
                    'host_disk_read_mbs': 6.0, 'host_disk_write_mbs': 3.0,
                    'emu_cpu_tot_emulator-5554': 51.0, 'emu_cpu_adj_emulator-5554': 26.0, 'emu_ram_emulator-5554': 51.0,
                    'qemu_cpu_emulator-5554': 16.0, 'qemu_ram_emulator-5554': 3001.0,
                    'emu_disk_pct_emulator-5554': 11.0, 'emu_errs_emulator-5554': 6.0, 'qemu_cs_emulator-5554': 101.0
                }
            ]
        }
        ...
        with patch('builtins.open', return_value=MagicMock(__enter__=MagicMock(return_value=mock_file))):
            with patch('emulator_resource_monitor.run_cmd', return_value="Android Debug Bridge version 1.0.41"):
                save_summary(mock_summary, 1010.0, save_csv=False, save_txt=True, out_dir='.')
    ...
    def test_graphs_generation_limits(self, mock_figure_class):
        ...
        mock_summary = {
            ...
            'emulators': {},
            'time_series': [
                {'time': 1000.0, 'host_cpu': 40.0},
                {'time': 1005.0, 'host_cpu': 50.0}
            ]
        }
    ...
    def test_print_dashboard(self):
        ...
        with patch('sys.stdout', mock_stdout):
            print_dashboard(
                ...
                netsim_cpu="2%",
                guest_results={"emulator-5554": ("Mock Device", "[TEST TAG]", "50%", "25%", "50% (2.0GB/4.0GB, 5 errs, 10% data)", "0.0 MB")},
                host_ram="30%",
                host_swap="0%",
                ...
                with patch('os.path.exists', return_value=True):
                    def mock_open(path, mode='r'):
                        if path == '/etc/os-release':
                            return MagicMock(__enter__=MagicMock(return_value=['PRETTY_NAME="Mock OS"\n']))
                        elif path == '/proc/cpuinfo':
                            return MagicMock(__enter__=MagicMock(return_value=['model name : Mock CPU\n']))
                    ...
                    with patch('builtins.open', side_effect=mock_open):
                        with patch('psutil.virtual_memory') as mock_mem:
                            ...
                            with patch('emulator_resource_monitor.HAS_GPUTIL', False):
                                with patch('emulator_resource_monitor.run_cmd', return_value="Mock GPU, 4096"):
    ...
    def test_get_gpu_stats_nvidia(self):
        ...
        with patch('platform.system', return_value='Linux'):
            with patch('emulator_resource_monitor.HAS_GPUTIL', False):
                with patch('emulator_resource_monitor.run_cmd', return_value="30, 1000\n"):
    ...
    def test_get_gpu_stats_linux_sysfs(self):
        ...
        with patch('platform.system', return_value='Linux'):
            with patch('emulator_resource_monitor.HAS_GPUTIL', False):
                with patch('emulator_resource_monitor.run_cmd', return_value=""):
                    with patch('os.path.exists', return_value=True):
                        with patch('builtins.open', return_value=MagicMock(__enter__=MagicMock(return_value=MagicMock(read=MagicMock(return_value="45\n"))))):
                            gpu_load, gpu_mem = get_gpu_stats()
...
"""
        with patch('emulator_resource_monitor.run_cmd', return_value=mock_output):
            from emulator_resource_monitor import update_emu_gpu
            update_emu_gpu()
            res = get_emu_gpu_stats({"emulator-5554": 1234, "emulator-5556": 5678})
    ...
    def test_find_adb_env(self):
        ...
        with patch('shutil.which', return_value=None):
            with patch('os.environ.get', side_effect=lambda k, d=None: '/path/to/sdk' if k == 'ANDROID_HOME' else None):
                with patch('os.path.exists', return_value=True):
                    res = find_adb()
    ...
    def test_main_trends(self):
        ...
        mock_host_stats = [
            ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {}),
            ("20%", {}, "4%", "40%", "0%", 0, 0, [], "20MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {})
        ]
        ...
        with patch('sys.stdout', mock_stdout):
            with patch('emulator_resource_monitor.get_devices', return_value={}):
                with patch('emulator_resource_monitor.get_host_stats', mock_host):
                    with patch('time.sleep', side_effect=mock_sleep):
                        with patch('sys.argv', ['emulator_resource_monitor.py', '--interval', '0.1', '--no-summary']):
                            with patch('os._exit', return_value=None):
                                try:
    ...
    def test_main_minimum_sleep(self):
        ...
        with patch('sys.stdout', mock_stdout):
            with patch('emulator_resource_monitor.get_devices', return_value={}):
                with patch('emulator_resource_monitor.get_host_stats', return_value=("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {})):
                    with patch('time.sleep', side_effect=mock_sleep):
                        with patch('time.monotonic', side_effect=lambda: next(gen)):
                            with patch('sys.argv', ['emulator_resource_monitor.py', '--interval', '1.0', '--no-summary']):
                                with patch('os._exit', return_value=None):
                                    try:
                                        main()
                                    except KeyboardInterrupt: pass
    ...
    def test_get_host_stats_paging(self, mock_system, mock_cpu, mock_swap, mock_mem, mock_temps, mock_disk, mock_proc_stats):
        mock_cpu.return_value = 50.0
        mock_mem.return_value = MagicMock(percent=60.0, used=6000000000, total=10000000000)
        mock_swap.return_value = MagicMock(percent=10.0, used=1000000000, total=10000000000)
    ...
    def test_process_cpu_initial_zero(self, mock_iter):
        ...
        mock_proc.num_ctx_switches.return_value = Ctx(voluntary=100, involuntary=50)
        ...
        qemu_stats, unmapped_qemu, netsim, netsim_ram, qemu_pids, qemu_avds = get_process_stats()
        self.assertEqual(qemu_stats["emulator-5554"][0], "0.0%") # First tick should be 0.0%

        qemu_stats, unmapped_qemu, netsim, netsim_ram, qemu_pids, qemu_avds = get_process_stats()
        self.assertEqual(qemu_stats["emulator-5554"][0], "15.0%") # Second tick should be mocked value
    ...
    def test_graphs_generation_flat_data(self, mock_figure_class):
        ...
        mock_summary = {
            ...
            'time_series': [
                {
                    'time': 1000.0, 'host_cpu': 40.0, 'host_ram': 60.0,
                },
                {
                    'time': 1005.0, 'host_cpu': 40.0, 'host_ram': 60.0,
                }
            ]
        }
        ...
        self.assertFalse(any("HOST CPU HISTORY CHART:" in line for line in written_lines))
    ...
    def test_df_parsing_variations(self):
        ...
        with patch('emulator_resource_monitor.run_cmd', side_effect=mock_run_cmd):
            mock_df_output = "/dev/block/dm-5   10G   2G   8G  20% /data"
            model, is_booting, g_diff, g_real, g_ram, swap = get_guest_stats("emulator-5554", "Android 12", False, "device")
    ...
    @patch('emulator_resource_monitor.get_devices')
    @patch('emulator_resource_monitor.get_host_stats')
    def test_main_loop_flaky_resolution_no_duplicates(self, mock_host, mock_devices, mock_guest, mock_save):
        from emulator_resource_monitor import main
        import time
        ...
        mock_host.side_effect = [
            ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {"emulator-5554": "Pixel_7"}),
            ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {}),
            ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {"emulator-5554": "Pixel_7"})
        ]

        mock_guest.return_value = ("Pixel_7", False, "10%", "5%", "50.0%", "0.0 MB")
    ...
    @patch('emulator_resource_monitor.get_devices')
    @patch('emulator_resource_monitor.get_host_stats')
    def test_main_loop_offline_start_tracks_boot(self, mock_host, mock_devices, mock_guest, mock_save):
        from emulator_resource_monitor import main
        import time
    ...
    @patch('emulator_resource_monitor.get_devices')
    @patch('emulator_resource_monitor.get_host_stats')
    def test_main_loop_boot_stats_duration_extended(self, mock_host, mock_devices, mock_guest, mock_save):
        from emulator_resource_monitor import main
        import time
    ...
    @patch('emulator_resource_monitor.get_devices')
    @patch('emulator_resource_monitor.get_host_stats')
    def test_boot_tracking_glitch_ignored(self, mock_host, mock_devices, mock_guest, mock_save, mock_pool):
        from emulator_resource_monitor import main
        import time
        ...
        mock_devices.return_value = {"emulator-5554": "device"}
        mock_host.return_value = ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {"emulator-5554": 1234}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {})
    ...
    @patch('emulator_resource_monitor.get_devices')
    @patch('emulator_resource_monitor.get_host_stats')
    def test_boot_tracking_ignores_flaky_is_booting(self, mock_host, mock_devices, mock_guest, mock_save, mock_pool):
        from emulator_resource_monitor import main
        import time
        from unittest.mock import MagicMock
        ...
        def mock_sleep(seconds):
            nonlocal iteration, current_time
            iteration += 1
            if iteration == 1: current_time = 200.0
            elif iteration == 2: current_time = 300.0
            elif iteration == 3: current_time = 700.0 # Expire window (700-300=400 > 300)
            elif iteration == 4: current_time = 800.0
    ...
    @patch('emulator_resource_monitor.get_devices')
    @patch('emulator_resource_monitor.get_host_stats')
    def test_boot_tracking_restart_detected(self, mock_host, mock_devices, mock_guest, mock_save, mock_pool):
        from emulator_resource_monitor import main
        import time
        from unittest.mock import MagicMock
        ...
        mock_host.side_effect = [
            ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {"emulator-5554": 1234}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {}),
            ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {"emulator-5554": 1234}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {}),
            ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {"emulator-5554": 5678}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", {})
        ]
        ...
        with patch('time.sleep', side_effect=mock_sleep):
            with patch('sys.argv', ['emulator_resource_monitor.py', '--interval', '0.1', '--no-summary', '--headless']):
                with patch('os._exit', return_value=None):
                    try:
                        main()
                    except KeyboardInterrupt: pass
    ...
    def test_csv_timestamp_calculation(self):
        ...
        mock_summary = {
            ...
            'netsim_ram_iterations': 0, 'netsim_ram_sum': 0.0, 'netsim_ram_max': 0.0,
            'emulators': {},
            'time_series': [{'time': 600.0, 'host_cpu': 45.0}]
        }

        with tempfile.TemporaryDirectory() as tmpdir:
            save_summary(mock_summary, 1005.0, save_csv=True, save_txt=False, out_dir=tmpdir)
            ...
            with open(os.path.join(tmpdir, csv_files[0]), 'r') as f:
                ...
                expected_time_str = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(1100.0))
    ...
    def test_process_stats_access_denied(self, mock_iter):
        import psutil
        mock_proc = MagicMock()
        mock_proc.info = {'name': 'qemu-system-x86_64', 'pid': 1234, 'create_time': 10000.0}
    ...
    def test_print_row_nan_placeholder(self):
        ...
        try:
            print_dashboard(
                "Linux", "10%", {}, "N/A",
                {"emulator-5554": ("Mock Device", "", "NaN%", "NaN%", "NaN%", "NaN")},
                "30%", "0%", "In: 0 / Out: 0", 1, 80, False, [], "N/A", "N/A", "N/A", {}, is_once=True
            )
        finally:
    ...
    def test_ansi_stripping_truncation(self):
        ...
        try:
            ...
            print_dashboard(
                "Linux", "10%", {}, "N/A",
                {"emulator-5554": ("Mock Device", "", "10%", "5%", ansi_val, "0.0 MB")},
                "30%", "0%", "In: 0 / Out: 0", 1, 80, False, [], "N/A", "N/A", "N/A", {}, is_once=True
            )
        finally:
            sys.stdout = sys.__stdout__
    ...
    def test_plotille_limits_floor(self):
        ...
        mock_summary = {
            'iterations': 1, 'start_time': 1000.0, 'start_time_monotonic': 1000.0,
            'host_cpu_iterations': 1, 'host_cpu_sum': 45.0, 'host_cpu_max': 45.0,
            'host_ram_iterations': 1, 'host_ram_sum': 60.0, 'host_ram_max': 60.0,
            'netsim_cpu_iterations': 0, 'netsim_cpu_sum': 0.0, 'netsim_cpu_max': 0.0,
            'netsim_ram_iterations': 0, 'netsim_ram_sum': 0.0, 'netsim_ram_max': 0.0,
            'emulators': {}, 'time_series': [{'time': 1000.0, 'host_cpu': 0.0}]
        }

        with patch('builtins.open', MagicMock()):
            try:
                save_summary(mock_summary, 1005.0, save_csv=False, save_txt=True, out_dir='.')
    ...
    def test_unmapped_qemu_ui_clamping(self):
        ...
        try:
            print_dashboard(
                "Linux", "10%", {}, "N/A", {}, "30%", "0%", "In: 0 / Out: 0", 1, 80, False, unmapped, "N/A", "N/A", "N/A", {}, is_once=True
            )
        finally:
            sys.stdout = sys.__stdout__
    ...
    def test_save_summary_qemu_gpu_chart(self):
        ...
        mock_summary = {
            'iterations': 2, 'start_time': 1000.0, 'start_time_monotonic': 1000.0,
            'host_cpu_iterations': 2, 'host_cpu_sum': 90.0, 'host_cpu_max': 50.0,
            'host_ram_iterations': 2, 'host_ram_sum': 120.0, 'host_ram_max': 60.0,
            'netsim_cpu_iterations': 0, 'netsim_cpu_sum': 0.0, 'netsim_cpu_max': 0.0,
            'netsim_ram_iterations': 0, 'netsim_ram_sum': 0.0, 'netsim_ram_max': 0.0,
            'emulators': {
                'emulator-5554': {
                    'name': 'Mock Device',
                    'cpu_sum': 100.0, 'cpu_iterations': 2,
                    'ram_sum': 100.0, 'ram_iterations': 2,
                    'ram_max': 50.0
                }
            },
            'time_series': [
                {'time': 1000.0, 'qemu_gpu_emulator-5554': 10.0},
                {'time': 1005.0, 'qemu_gpu_emulator-5554': 20.0}
            ]
        }

        with tempfile.TemporaryDirectory() as tmpdir:
            save_summary(mock_summary, 1010.0, save_csv=False, save_txt=True, out_dir=tmpdir)
    ...
    def test_file_generation_shared_timestamp(self):
        ...
        mock_summary = {
            ...
            'netsim_cpu_iterations': 0, 'netsim_cpu_sum': 0.0, 'netsim_cpu_max': 0.0,
            'netsim_ram_iterations': 0, 'netsim_ram_sum': 0.0, 'netsim_ram_max': 0.0,
            'emulators': {}, 'time_series': [{'time': 1000.0}]
        }

        with tempfile.TemporaryDirectory() as tmpdir:
            save_summary(mock_summary, 1005.0, save_csv=True, save_txt=True, out_dir=tmpdir)
    ...
    @patch('emulator_resource_monitor.get_devices')
    @patch('emulator_resource_monitor.get_host_stats')
    def test_avd_key_collision(self, mock_host, mock_devices, mock_guest, mock_save):
        from emulator_resource_monitor import main
        import time

        # Mock two connected devices with same AVD name but different serials
        mock_devices.return_value = {"emulator-5554": "device", "emulator-5556": "device"}

        # Mock host stats returning same AVD name for both serials
        qemu_avds = {"emulator-5554": "Pixel_6_API_31", "emulator-5556": "Pixel_6_API_31"}
        mock_host.return_value = ("10%", {}, "2%", "30%", "0%", 0, 0, [], "10MB", {}, "0%", "0%", {}, "N/A", "0MB/s", 0.0, 0.0, "N/A", qemu_avds)

        # Mock guest stats
        mock_guest.return_value = ("Pixel_6_API_31", False, "10%", "5%", "50.0%", "0.0 MB")
...
class TestCacheInvalidation(unittest.TestCase):
    ...
    def test_normal_operation(self):
        ...
        update_device_cache(device_names, disconnect_times, serial_pids, connected_serials, qemu_pids, 1000.0)
    ...
    def test_disconnect_starts_timer(self):
        ...
        self.assertEqual(device_names, {'emulator-5554': 'Model A'}) # Still there
        self.assertEqual(disconnect_times, {'emulator-5554': 1000.0}) # Timer started
    ...
    def test_disconnect_timeout_expires(self):
        ...
        update_device_cache(device_names, disconnect_times, serial_pids, connected_serials, qemu_pids, 1006.0) # Delta > 5
    ...
    def test_disconnect_waiting(self):
        ...
        update_device_cache(device_names, disconnect_times, serial_pids, connected_serials, qemu_pids, 1004.0) # Delta <= 5
    ...
    def test_reconnect_before_timeout(self):
        ...
        update_device_cache(device_names, disconnect_times, serial_pids, connected_serials, qemu_pids, 1004.0)

        self.assertEqual(disconnect_times, {}) # Cleared
        self.assertEqual(device_names, {'emulator-5554': 'Model A'}) # Preserved

...
PUBLIC_VERIFIER_URLS = {
    ...
    "x86_64": "https://dl.google.com/dl/android/cts/android-cts-verifier-17_r1-linux_x86-x86.zip",
    "arm": "https://dl.google.com/dl/android/cts/android-cts-verifier-17_r1-linux_x86-arm.zip",
    "arm64": "https://dl.google.com/dl/android/cts/android-cts-verifier-17_r1-linux_x86-arm.zip",
}
...
def discover_available_modules(scripts_dir):
    ...
    for script_name in os.listdir(scripts_dir):
        if not script_name.startswith("pass_") or not script_name.endswith(".py"):
            continue
        ...
        if len(short_parts) > 1:
            ...
            if len(short_parts) >= 3 and short_parts[0] in ("other", "tiles", "projection", "sharesheet", "vibrations"):
...
def register_parser(subparsers):
    ...
    cts_subparsers = cts_parser.add_subparsers(dest="cts_cmd", help="Available CTS subcommands")
    cts_parser.set_defaults(parser=cts_parser, func=run_cts_help)

    verifier_parser = cts_subparsers.add_parser(
        "run-cts-verifier",
        help="Download CTS-Verifier package (public release or go/ab build ID) and execute automated tests"
    )
    verifier_parser.add_argument(
        "--build-id",
        type=str,
        default=None,
        help="Optional numeric build ID from Android Build (go/ab). If omitted, downloads latest public release from Google CDN"
    )
    verifier_parser.add_argument(
        "--module",
        type=str,
        default=None,
        help="Module name to test (run --list-modules to discover available test scripts dynamically)"
    )
    verifier_parser.add_argument(
        "--all",
        action="store_true",
    ...
    verifier_parser.add_argument(
        ...
        type=str,
        default=None,
        help="ADB device serial number (auto-detected via 'adb devices' if omitted)"
    )
    verifier_parser.add_argument(
        "--branch",
        type=str,
    ...
    verifier_parser.add_argument(
        ...
        type=str,
        default=None,
        help="Build target when using --build-id (defaults to test_suites_x86_64 or test_suites_arm64)"
    )
    verifier_parser.add_argument(
        "--force-download",
        action="store_true",
...
def download_with_progress(url, dest_path):
    ...
    def reporthook(blocknum, blocksize, totalsize):
        ...
        if totalsize > 0:
            ...
            sys.stdout.write(f"\rDownloading... {percent:5.1f}% [{mb_read:.1f}MB / {mb_total:.1f}MB]")
            sys.stdout.flush()
...
def fetch_from_android_build(build_id, branch, target, arch, dest_zip):
    ...
    if not fetch_tool:
        raise RuntimeError("Neither /google/data/ro/projects/android/fetch_artifact nor 'fetch_artifact' in PATH was found.")
    target_name = target or ("test_suites_arm64" if arch == "arm64" else "test_suites_x86_64")
    cmd = [fetch_tool, "--bid", build_id, "--branch", branch, "--target", target_name, "android-cts-verifier.zip", dest_zip]
    print(f"Fetching CTS-Verifier from Android Build (bid={build_id}, branch={branch}, target={target_name})...")
    res = subprocess.run(cmd, check=False)
    if res.returncode != 0:
        raise RuntimeError(f"fetch_artifact failed with exit code {res.returncode}")
...
def run_cts_verifier(args):
    ...
    if args.list_modules:
        if not json_mode:
            print(f"Dynamically Discovered Automated CTS-Verifier Modules ({len(discovered_modules)} modules found):")
            print(f"Source directory: {scripts_dir or 'Not found'} (Resolved via {origin_info or 'Unknown'})\n")
    ...
    if not serial and not args.list_modules:
        print_result({
            "status": "error",
            "action": "run-cts-verifier",
            "error_message": "No online ADB device/emulator found. Launch an emulator first or pass --serial.",
            "exit_code": 1
        }, json_mode=json_mode, is_error=True)
        sys.exit(1)
    ...
    if args.build_id:
        ...
        extracted_apk = os.path.join(cache_dir, "extracted", "android-cts-verifier", "CtsVerifier.apk")
    else:
    ...
    if args.force_download or not os.path.exists(extracted_apk):
        ...
        if args.build_id:
            fetch_from_android_build(args.build_id, args.branch, args.target, arch_key, dest_zip)
        else:
    ...
    inst_res = subprocess.run(adb_prefix + ["install", "-r", "-g", extracted_apk], capture_output=True, text=True, check=False)
    ...
    subprocess.run(adb_prefix + ["shell", "appops", "set", "com.android.cts.verifier", "android:read_device_identifiers", "allow"], check=False)
    subprocess.run(adb_prefix + ["shell", "appops", "set", "com.android.cts.verifier", "MANAGE_EXTERNAL_STORAGE", "0"], check=False)
    subprocess.run(adb_prefix + ["shell", "appops", "set", "com.android.cts.verifier", "TURN_SCREEN_ON", "0"], check=False)
    subprocess.run(adb_prefix + ["shell", "am", "compat", "enable", "ALLOW_TEST_API_ACCESS", "com.android.cts.verifier"], check=False)

    # Step 3: Run requested test module(s)
    if not scripts_dir:
        raise RuntimeError("Could not find third_party/adt-infra/goldfish_test/xts/verifier/ scripts directory.")

    if args.all:
        # Run all unique python test scripts
        unique_scripts = sorted(set(discovered_modules.values()))
        print("-" * 50)
        print(f"🚀 Running ALL {len(unique_scripts)} CTS-Verifier automated modules...")
        print("-" * 50)
        results = []
        overall_failed = 0
        for idx, script_file in enumerate(unique_scripts, 1):
            ...
            run_res = subprocess.run([sys.executable, script_path], cwd=scripts_dir, env=env, check=False)
            passed = run_res.returncode == 0
            if not passed:
                overall_failed += 1
            results.append({"module": mod_label, "script": script_file, "passed": passed})
    ...
    if not script_file and scripts_dir:
        candidate_file = os.path.join(scripts_dir, module_name if module_name.endswith(".py") else f"{module_name}.py")
    ...
    if not script_file:
        print_result({
            "status": "error",
            "action": "run-cts-verifier",
            "error_message": f"Unknown module '{args.module}'. Run with --list-modules to see all dynamically discovered tests.",
            "exit_code": 1
        }, json_mode=json_mode, is_error=True)
        sys.exit(1)
    ...
    passed = run_res.returncode == 0
    xml_files = glob.glob(os.path.join(out_dir, "**", "test_result.xml"), recursive=True)
    ...
    print_result({
        ...
        "verifier_apk": extracted_apk,
        "source_mode": f"go/ab build-id {args.build_id}" if args.build_id else "public CDN 17_r1",
